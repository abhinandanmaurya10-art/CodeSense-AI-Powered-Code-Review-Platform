...
const defaultApiUrl = typeof window !== "undefined" ? normalizeApiUrl(`${window.location.origin}/api`) : "";
const API_BASE_URL = normalizeApiUrl(defaultApiUrl);
async function request(path, options = {}, token) {
  if (!API_BASE_URL) {
    throw new Error(
      "The production API URL is not configured. Set VITE_API_URL in Vercel to your backend API endpoint ending with /api, or deploy the backend under the same domain so /api is available."
    );
  }
